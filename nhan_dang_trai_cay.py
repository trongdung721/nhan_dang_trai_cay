...
if img_file_buffer is not None:
    ...
    frame = cv2.cvtColor(frame, cv2.COLOR_RGB2BGR) # BGR -> RGB
    ...
    if st.button('Predict'):
        if not frame is None:
                    ...
                    inpWidth = mywidth if mywidth else frameHeight
                    inpHeight = myheight if myheight else frameHeight
                    blob = cv2.dnn.blobFromImage(frame, size=(inpWidth, inpHeight), swapRB=rgb, ddepth=cv2.CV_8U)
